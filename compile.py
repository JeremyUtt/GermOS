...
os.system(f'/usr/local/i386elfgcc/bin/i386-elf-ld -o build/Linking_Stage_1.bin -Ttext 0x1000 build/kernel_entry.o build/Uni2-Terminus12x6.o build/zap-light20.o {files} --oformat binary')
print(f'/usr/local/i386elfgcc/bin/i386-elf-ld -o build/Linking_Stage_1.bin -Ttext 0x1000 build/kernel_entry.o build/Uni2-Terminus12x6.o build/zap-light20.o {files} --oformat binary')
print("")
# For Debugger: 
os.system(f'/usr/local/i386elfgcc/bin/i386-elf-ld -o "build/Linking_Stage_1.o" -Ttext 0x1000 "build/kernel_entry.o" "build/Uni2-Terminus12x6.o" "build/zap-light20.o" {files} ')
os.system('objcopy --only-keep-debug build/Linking_Stage_1.o bin/Linking_Stage_1.sym')

# print("====> Adding Bootloader and Finishing up...")
os.system('cat "build/boot.bin" "build/Linking_Stage_1.bin" > "build/Linking_Stage_2.bin"')
os.system('cat "build/Linking_Stage_2.bin" "src/zeros.bin" > "bin/OS.bin"')
