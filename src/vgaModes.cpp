...
void _vmemwr(unsigned seg, unsigned dst_off, unsigned char *src, unsigned count) {
    // Calculate the linear address
    uint32_t address = (seg << 4) + dst_off;
    // Copy the data to the destination address byte by byte
    for (unsigned i = 0; i < count; ++i) {
        *((volatile uint8_t *)(address + i)) = src[i];
    }
}
void pokeb(unsigned seg, unsigned off, unsigned val) {
    // Calculate the linear address
    uint32_t address = (seg << 4) + off;
...
unsigned char g_320x200x256_real[] = {
/* MISC */
        0x63,
/* SEQ */
        0x3, 0x1, 0xf, 0x0, 0xe, 
/* CRTC */
        ...
        0xff, 
/* GC */
        0x0, 0x0, 0x0, 0x0, 0x0, 0x40, 0x5, 0xf,
        0xff, 
/* AC */
        0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7,
        0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf,
        0x41, 0x0, 0xf, 0x0, 0x0, 
};
...
void dump_regs(unsigned char *regs) {
    fprintf(Serial, "unsigned char g_mode[] =\n");
    fprintf(Serial, "{\n");
